...
public:
  /**
   * Create StreamingClientHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  StreamingClientHelper (uint16_t port);
...
public:
  /**
   * Create StreamingStreamerHelper which will make life easier for people trying
   ...
   */
  StreamingStreamerHelper (Address addr);
  ...
  /**
   * Given a pointer to a StreamingStreamer application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be StreamingStreamer).
   ...
   *
   * \param node The Ptr<Node> on which to create the StreamingStreamerApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the StreamingStreamerApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::StreamingStreamer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an StreamingStreamer will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.

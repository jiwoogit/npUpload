...
StreamingClientHelper::StreamingClientHelper (uint16_t port)
{
...
StreamingStreamerHelper::StreamingStreamerHelper (Address address)
{
  m_factory.SetTypeId (StreamingStreamer::GetTypeId ());
  SetAttribute ("RemoteAddress", AddressValue (address));
}
...
void 
StreamingStreamerHelper::SetAttribute (
  std::string name, 
  const AttributeValue &value)
{
  m_factory.Set (name, value);
...
void
StreamingStreamerHelper::SetFill (Ptr<Application> app, std::string fill)
{
  app->GetObject<StreamingStreamer>()->SetFill (fill);
}

void
StreamingStreamerHelper::SetFill (Ptr<Application> app, uint8_t fill, uint32_t dataLength)
{
  app->GetObject<StreamingStreamer>()->SetFill (fill, dataLength);
}

void
StreamingStreamerHelper::SetFill (Ptr<Application> app, uint8_t *fill, uint32_t fillLength, uint32_t dataLength)
{
  app->GetObject<StreamingStreamer>()->SetFill (fill, fillLength, dataLength);
}
...
ApplicationContainer
StreamingStreamerHelper::Install (std::string nodeName) const
{
  Ptr<Node> node = Names::Find<Node> (nodeName);
  return ApplicationContainer (InstallPriv (node));
}
...
ApplicationContainer
StreamingStreamerHelper::Install (NodeContainer c) const
{
  ApplicationContainer apps;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
